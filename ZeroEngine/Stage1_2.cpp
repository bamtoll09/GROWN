	...
	:bulletTime(0.f), theWorld(0.f), isTheWorlded(false), db(0), speed(300), play(true), playTime(0.f)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		tile1[i]->SetPos(949 + i * 80, 704);
		PushScene(tile1[i]);
	}
	for (int i = 0; i < 3; i++)
	{
		tile2[i] = new Tile(m_pPlayer, 1);
		tile2[i]->SetPos(597 + i * 80, 704);
		PushScene(tile2[i]);
	}
	for (int i = 0; i < 3; i++)
	{
		tile3[i] = new Tile(m_pPlayer, 1);
		tile3[i]->SetPos(261 + i * 80, 704);
		PushScene(tile3[i]);
	}
	for (int i = 0; i < 2; i++)
	{
		tile4[i] = new Tile(m_pPlayer, 1);
		tile4[i]->SetPos(i * 80, 704);
		PushScene(tile4[i]);
	}
	...
	for (int i = 0; i < 4; i++)
	{
		...
		ladder[i]->SetPos(1110, 704 + i * 80);
		PushScene(ladder[i]);
	}

	door[0] = new Door(2);
	door[0]->SetPos(38, 818);

	door[1] = new Door(4);
	door[1]->SetPos(0, tile4[0]->Pos().y - door[1]->Height());

	monster = new Monster(m_pPlayer, 1);
	monster->SetPos(765, 890);
...
void Stage1_2::Update(float eTime)
{
	...
	if (m_pPlayer->Pos().x + m_pPlayer->Width() - 40 >= tile4[0]->Pos().x && m_pPlayer->Pos().x + 40 <= tile4[1]->Pos().x + tile4[0]->Width()
	...
	else if (m_pPlayer->Pos().x + m_pPlayer->Width() - 40 >= tile3[0]->Pos().x && m_pPlayer->Pos().x + 40 <= tile3[2]->Pos().x + tile3[0]->Width()
	...
	else if (m_pPlayer->Pos().x + m_pPlayer->Width() - 40 >= tile2[0]->Pos().x && m_pPlayer->Pos().x + 40 <= tile2[2]->Pos().x + tile2[0]->Width()
	...
	else if (m_pPlayer->Pos().x + m_pPlayer->Width() - 40 >= tile1[0]->Pos().x && m_pPlayer->Pos().x + 40 <= tile1[3]->Pos().x + tile1[0]->Width()
...
bool Stage1_2::rectangleCollision(Player *_r1, ZeroIScene *_r2) // 플레이어 충돌 판정
{
